...
class SfzListComponent: public Component, public TableListBoxModel
{
public:
	SfzListComponent(Configuration& config);
	void paint(Graphics & g);
...
private:
	Configuration& config;
	SfzFileChooser fileBrowser { config };
	SimpleVisibilityWatcher<SfzFileChooser> watcher { fileBrowser, [this] () { this->refreshList(); } };
	TableListBox sfzList { "Sfz List", this };
	TextButton addButton;
	TextButton closeButton;
	void refreshList();
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(SfzListComponent)
};